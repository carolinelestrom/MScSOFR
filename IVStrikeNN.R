...
SOFRSpot_Smile_Normal <- function(S, t, T, jumps, jumps_, thetat, xit, rt, st, intS_t){
  1/(T - S) * (rt * (T - S) +                                                         ### Constant Rate Contribution
                 ...
                 1/2 * (T - S)^2 * nu_P * mu_P +                                      ### Unscheduled Jumps Contribution
                 min(T - S, T - jumps[1]) * MeanXi(t, jumps_[1], thetat, xit) +       ### 1st Scheduled Jump
                 min(T - S, T - jumps[2]) * MeanXi(t, jumps_[2], thetat, xit) +       ### 2nd Scheduled Jump
                 min(T - S, T - jumps[3]) * MeanXi(t, jumps_[3], thetat, xit) +       ### 3rd Scheduled Jump
                 min(T - S, T - jumps[4]) * MeanXi(t, jumps_[4], thetat, xit) +       ### 4th Scheduled Jump
                 min(T - S, T - jumps[5]) * MeanXi(t, jumps_[5], thetat, xit) +       ### 5th Scheduled Jump
                 min(T - S, T - jumps[6]) * MeanXi(t, jumps_[6], thetat, xit) +       ### 6th Scheduled Jump
                 min(T - S, T - jumps[7]) * MeanXi(t, jumps_[7], thetat, xit) +       ### 7th Scheduled Jump
                 min(T - S, T - jumps[8]) * MeanXi(t, jumps_[8], thetat, xit) +       ### 8th Scheduled Jump
                 intS_t)                                                              ### Integral from S to t
}


### Strike
K_Smile_Normal <- 100 * (1 - SOFRSpot_Smile_Normal(S = S, t = 0, T = T, jumps = jumps, jumps_ = jumps_, thetat = theta0, xit = xi0, rt = r0, st = s0, intS_t = 0))
k_Smile_Normal <- (T - S) * (100 - K_Smile_Normal) / 100
...
q_hat_Smile_Normal <- function(w, S, t, T, jumps, jumps_, K, ZCB, thetat, xit, rt, st, intS_t){ 
  ...
  FirstExpectation <- exp(-((S - jumps[1]) + min(T - S, T - jumps[1]) * (1 + x)) * MeanXi(t, jumps_[1], thetat, xit)) *                                           ### 1st jump
    exp((-((S - jumps[1]) + min(T - S, T - jumps[1]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(t, jumps_[1]) %*% Gamma_Q)) *                ### 1st jump
    exp(-((S - jumps[2]) + min(T - S, T - jumps[2]) * (1 + x)) * MeanXi(t, jumps_[2], thetat, xit)) *                                                             ### 2nd jump
    exp((-((S - jumps[2]) + min(T - S, T - jumps[2]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[1], jumps_[2]) %*% Gamma_Q)) *        ### 2nd jump
    exp(-((S - jumps[3]) + min(T - S, T - jumps[3]) * (1 + x)) * MeanXi(t, jumps_[3], thetat, xit)) *                                                             ### 3rd jump
    exp((-((S - jumps[3]) + min(T - S, T - jumps[3]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[2], jumps_[3]) %*% Gamma_Q)) *        ### 3rd jump
    exp(-((S - jumps[4]) + min(T - S, T - jumps[4]) * (1 + x)) * MeanXi(t, jumps_[4], thetat, xit)) *                                                             ### 4th jump
    exp((-((S - jumps[4]) + min(T - S, T - jumps[4]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[3], jumps_[4]) %*% Gamma_Q)) *        ### 4th jump
    exp(-((S - jumps[5]) + min(T - S, T - jumps[5]) * (1 + x)) * MeanXi(t, jumps_[5], thetat, xit)) *                                                             ### 5th jump
    exp((-((S - jumps[5]) + min(T - S, T - jumps[5]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[4], jumps_[5]) %*% Gamma_Q)) *        ### 5th jump
    exp(-((S - jumps[6]) + min(T - S, T - jumps[6]) * (1 + x)) * MeanXi(t, jumps_[6], thetat, xit)) *                                                             ### 6th jump
    exp((-((S - jumps[6]) + min(T - S, T - jumps[6]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[5], jumps_[6]) %*% Gamma_Q)) *        ### 6th jump
    exp(-((S - jumps[7]) + min(T - S, T - jumps[7]) * (1 + x)) * MeanXi(t, jumps_[7], thetat, xit)) *                                                             ### 7th jump
    exp((-((S - jumps[7]) + min(T - S, T - jumps[7]) * (1 + x)))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[6], jumps_[7]) %*% Gamma_Q)) *        ### 7th jump
    exp(-min(T - S, T - jumps[8]) * (1 + x) * MeanXi(t, jumps_[8], thetat, xit)) *                                                                                ### 8th jump in reference period
    exp((-min(T - S, T - jumps[8]) * (1 + x))^2 / 2 * (omega^2 + t(Gamma_Q) %*% VarX(jumps_[7], jumps_[8]) %*% Gamma_Q))                             ### 8th jump in reference period
...
####################################################################################################################
####################################################################################################################
price1MSOFROption_Smile_Normal <- function(S, t, T, jumps, jumps_, K, ZCB, thetat, xit, rt, st, intS_t) { 
  umax <- 50000
  integrand_q_hat <- function(w) {
    Re(q_hat_Smile_Normal(w, S = S, t = t, T = T, jumps = jumps, jumps_ = jumps_, K = K, ZCB = ZCB, thetat = thetat, xit = xit, rt = rt, st = st, intS_t = intS_t) / 
...
CallSmile_Normal <- rep(NA, length(StrikeSmile))
IVSmile_Normal <- rep(NA, length(StrikeSmile))
for (i in 1:length(StrikeSmile)) {
  print(i)
  CallSmile_Normal[i] <- price1MSOFROption_Smile_Normal(S = S, t = 0, T = T, jumps = jumps, jumps_ = jumps_,
                                                        K = StrikeSmile[i], ZCB_s_NN(t = 0, T = T, thetat = theta0, xit = xi0, rt = r0, st = s0),
                                                        thetat = theta0, xit = xi0, rt = r0, st = s0, intS_t = 0)
  IVSmile_Normal[i] = IVTest(spot = K_Smile_Normal, strike = StrikeSmile[i],
                             t = 0, T = T, ZCB_s_NN(t = 0, T = T, thetat = theta0, xit = xi0, rt = r0, st = s0), CallSmile_Normal[i])
}
