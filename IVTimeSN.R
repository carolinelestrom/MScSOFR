...
SOFRSpot_NormalSkellam <- function(S, t, T, jump, jump_, zetat, rt, st, intS_t){
  if (t < jump){
...
### Strike
K_NormalSkellam <- 100 * (1 - SOFRSpot_NormalSkellam(S = 0, t = 0, T = T, jump = jump, jump_ = jump_, zetat = zeta0, rt = r0, st = s0, intS_t = 0))
k_NormalSkellam <- (T - S) * (100 - K_NormalSkellam) / 100
...
q_hat_NormalSkellam <- function(w, S, t, T, jump, jump_, K, ZCB, zetat, rt, st, intS_t){ 
  ...
  if (t < jump){
    ### MGF for non-central chi-squared
...
####################################################################################################################
####################################################################################################################
price1MSOFROption_NormalSkellam <- function(S, t, T, jump, jump_, K, ZCB, zetat, rt, st, intS_t) { 
  umax <- 50000
  integrand_q_hat <- function(w) {
    Re(q_hat_NormalSkellam(w, S = S, t = t, T = T, jump = jump, jump_ = jump_, K = K, ZCB = ZCB, zetat = zetat, rt = rt, st = st, intS_t = intS_t) / 
...
J_D <- 0
N_D <- 0
IV_BS_Bach_NormalSkellam <- IV_BS_NormalSkellam <- numeric(nsteps)
S0_NormalSkellam <- numeric(nsteps)
...
for (i in 1:(nsteps - 10)) {  ### Avoid issues in the last steps
  ...
  ### Call Price
  Call <- price1MSOFROption_NormalSkellam(S, t, T, jump, jump_, K_NormalSkellam, ZCB_s_SN(t, T, zetat = zetam, rt = rm, sm), zetat = zetam, rt = rm, st = sm, intS_t = int_sm)
  
  ### Spots
  S0_NormalSkellam[i] <- 100 * (1 - SOFRSpot_NormalSkellam(S, t, T, jump, jump_, zetat = zetam, rt = rm, st = sm, intS_t = int_sm))
  
  ### Implicit Volatility
  IV_BS_NormalSkellam[i] <- IVTest(S0_NormalSkellam[i], K_NormalSkellam, t, T, ZCB_s_SN(t, T, zetam, rm, sm), Call)
}

### Plotting the results
plot(S0_NormalSkellam[1:(nsteps - 100)], col = "steelblue")
plot(seq(0, T, length.out = nsteps), IV_BS_Bach_NormalSkellam, type = "l", col = "steelblue", 
     xlab = "t", ylab = "Implied Volatility", main = "Imp Black Vol for t in [0, 1/12]")
abline(v = (10 - 0)/360, lty = 2)
plot(seq(0, T, length.out = nsteps), IV_BS_NormalSkellam, type = "l", col = "steelblue", 
     xlab = "t", ylab = "Implied Volatility", main = "Imp Black Vol for t in [0, 1/12]")
abline(v = (10 - 0)/360, lty = 2)
plot(seq(0, T, length.out = nsteps), IV_BS_NormalSkellam, type = "l", col = "steelblue", 
