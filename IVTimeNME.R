...
SOFRSpot_MixedExp <- function(S, t, T, jump, jump_, thetat, xit, rt, st, intS_t){
  if (t < jump){
...
### Strike
K_MixedExp <- 100 * (1 - SOFRSpot_MixedExp(S = 0, t = 0, T = T, jump = jump, jump_ = jump_, thetat = theta0, xit = xi0, rt = r0, st = s0, intS_t = 0))
k_MixedExp <- (T - S) * (100 - K_MixedExp) / 100
...
q_hat_MixedExp <- function(w, S, t, T, jump, jump_, K, ZCB, thetat, xit, rt, st, intS_t){ 
...
####################################################################################################################
####################################################################################################################
price1MSOFROption_MixedExp <- function(S, t, T, jump, jump_, K, ZCB, thetat, xit, rt, st, intS_t) { 
  umax <- 50000
  integrand_q_hat <- function(w) {
    Re(q_hat_MixedExp(w, S = S, t = t, T = T, jump = jump, jump_ = jump_, K = K, ZCB = ZCB, thetat = thetat, xit = xit, rt = rt, st = st, intS_t = intS_t) / 
...
J_D <- 0
N_D <- 0
IV_BS_Bach_MixedExp <- IV_BS_MixedExp <- numeric(nsteps)
S0_MixedExp <- numeric(nsteps)
...
for (i in 1:(nsteps - 10)) {  ### Avoid issues in the last steps
  ...
  #J_D <- rnorm(1, mean = 0.003, sd = 0.2)
  
  if (t == jump) { #(abs(t - jump) < 1e-4) #(t == jump) #(time_series[i] %in% jump_dates) #(is_close(t, jump_dates))
  ...
  ### Call Price
  Call <- price1MSOFROption_MixedExp(S, t, T, jump, jump_, K_MixedExp, ZCB_s_NME(t, T, thetat = thetam, xit = xim, rt = rm, sm), thetat = thetam, xit = xim, rt = rm, st = sm, intS_t = int_sm)
  
  ### Spots
  S0_MixedExp[i] <- 100 * (1 - SOFRSpot_MixedExp(S, t, T, jump, jump_, thetat = thetam, xit = xim, rt = rm, st = sm, intS_t = int_sm))
  
  ### Implicit Volatility
  IV_BS_MixedExp[i] <- IVTest(S0_MixedExp[i], K_MixedExp, t, T, ZCB_s_NME(t, T, thetam, xim, rm, sm), Call)
}



plot(seq(0, 1/12, length.out = nsteps), IV_BS_MixedExp, type = "l", col = "#901a1E", lwd = 1, ylim = c(0, 0.1),
...
plot(S0_MixedExp[1:(nsteps - 100)], col = "steelblue")
plot(seq(0, T, length.out = nsteps), IV_BS_Bach_MixedExp, type = "l", col = "steelblue", 
     xlab = "t", ylab = "Implied Volatility", main = "Imp Black Vol for t in [0, 1/12]")
abline(v = (10 - 0)/360, lty = 2)
plot(seq(0, T, length.out = nsteps), IV_BS_MixedExp, type = "l", col = "steelblue", 
     xlab = "t", ylab = "Implied Volatility", main = "Imp Black Vol for t in [0, 1/12]")
abline(v = (10 - 0)/360, lty = 2)
plot(seq(0, T, length.out = nsteps), IV_BS_MixedExp, type = "l", col = "steelblue", 
